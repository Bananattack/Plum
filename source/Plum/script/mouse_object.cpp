...
namespace Plum
{
	...
	{
		lua_pushnumber(L, Script::getInstance(L)->engine->mouse.x);
		return 1;
	}

	static int mouseGetY(lua_State* L)
	{
		lua_pushnumber(L, Script::getInstance(L)->engine->mouse.y);
		return 1;
	}

	static int mouseGetWheelPosition(lua_State* L)
	{
		lua_pushinteger(L, Script::getInstance(L)->engine->mouse.wheelPosition);
		return 1;
	}
	...
	static int mouseSetWheelPosition(lua_State* L)
	{
		...
		Script::getInstance(L)->engine->mouse.wheelPosition = value;
		return 0;
	}
	...
	static int mouseGetLeft(lua_State* L)
	{
		...
		if(inputConstructor)
		{
			...
			lua_pushlightuserdata(L, &(Script::getInstance(L)->engine->mouse.button[MOUSE_LEFT]));
			lua_call(L, 1, 1);
			return 1;
		}
	...
	static int mouseGetMiddle(lua_State* L)
	{
		...
		{
			lua_pushcfunction(L, inputConstructor);
			lua_pushlightuserdata(L, &(Script::getInstance(L)->engine->mouse.button[MOUSE_MIDDLE]));
			lua_call(L, 1, 1);
			return 1;
		}
	...
	static int mouseGetRight(lua_State* L)
	{
		...
		{
			lua_pushcfunction(L, inputConstructor);
			lua_pushlightuserdata(L, &(Script::getInstance(L)->engine->mouse.button[MOUSE_RIGHT]));
			lua_call(L, 1, 1);
			return 1;
		}
	...
	void Script::initMouseClass(lua_State* L)
	{
