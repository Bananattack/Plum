...
namespace Plum
{
	...
	{
		lua_pushinteger(L, Script::getInstance(L)->engine->timer.gap);
		return 1;
	}

	static int timerGetFPS(lua_State* L)
	{
		lua_pushinteger(L, Script::getInstance(L)->engine->timer.fps);
		return 1;
	}

	static const luaL_Reg timerMembers[] = {
	...
	void Script::initTimerClass(lua_State* L)
	{
