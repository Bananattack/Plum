...
float AudioClip::load(Audio &audio, Sound sound, float limit)
{
	...
	while (total < capacity)
	{
		...
		AudioChunk chunk(audio, data->format, chanPtr, amt, frame, 0.0f, 1.0f);
		source.tick(frame);
		source.pull(chunk);
		std::cout << ".";
...
AudioClip::Player::Player(AudioClip *_clip, bool _loop) :
	loop(_loop)
{
	if (!_clip)
		reportError(L"Tried to create AudioClip::Player from null clip!");
	clip = Ref<AudioClip>(_clip);
...
void AudioClip::Player::pull(AudioChunk &chunk)
{
	...
	Uint32 chans = clip->data->format.channels;
	if (!link[0] && !pos)
	{
		for (Uint32 c = 0; c < chans; ++c) link[c] = clip->data->root[c];
	}

	Uint32 index = pos % (Link::SIZE/4), end = clip->data->length,
