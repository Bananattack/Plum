...
namespace plaidgadget
{
	...
	*/
	...
	class Bandpass_Node {public: Bandpass_Node(float l=0.0f,float h=0.0f) :
		low(l), high(h) {} float low, high;};
	/*
		A utility which can be used efficiently as a low-pass, high-pass or
			band-pass filter for listening purposes.

		When lowpass is "off", its cutoff frequency is set to 40,000.
		When highpass is "off", its cutoff frequency is set to 10.
		Highpass occurs before lowpass.
	*/
	class Bandpass : public AudioEffect<Bandpass_Node>
	{
	public:
		//Bind to signal
		Bandpass(Signal source, float low=0.0f, float high=0.0f);
		virtual ~Bandpass();
		...
		void lowpass(float freq);
		void highpass(float freq);
		void off();

	protected:
		virtual void pull(AudioChunk &chunk,
			const Bandpass_Node &a, const Bandpass_Node &b);
		virtual Bandpass_Node interpolate(
