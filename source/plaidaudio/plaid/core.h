...
//Debug control
#define reportError(X) plaidgadget::_reportError(X, __FILE__, __LINE__)
#define reportWarning(X) plaidgadget::_reportWarning(X, __FILE__, __LINE__)
#define pgNote(X) plaidgadget::_reportNote(X)
...
namespace plaidgadget
{
	...
	/*
		...
		For classes that can't afford the memory overhead of a vptr, you can
			create a public static member _type and a public method type()
