...
std::string plaidgadget::ToStdString(const String &str)
{
	std::string res(str.length(), ' ');
	std::copy(str.begin(), str.end(), res.begin());
	return res;
}
String plaidgadget::ToPGString(const std::string &str)
{
...
std::string plaidgadget::UTF8Encode(const String &str, char err)
{
...
String plaidgadget::UTF8Decode(const std::string &str, Char err)
{
	const char *p = str.c_str();
	String res;
